...
def extract_think_section(response):
    if "<think>" in response and "</think>" in response:
        start = response.find("<think>") + len("<think>")
        end = response.find("</think>")
        thinking = response[start:end].strip()
        answer = response[:response.find("<think>")].strip() + " " + response[response.find("</think>") + len("</think>"):].strip()
...
if uploaded_file:
    ...
    if question:
        ...
        response = answer_question(question, related_documents)
        
        thinking, answer = extract_think_section(response)
        ...
        with st.chat_message("assistant"):
            ...
            if thinking:
                with st.expander("Show thinking process"):
                    st.write(thinking)
            with st.expander("Show related excerpts"):
